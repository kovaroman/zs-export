...
class FashionDE extends CSVGenerator
{
	...
	private function getMain(Record $item, KeyValue $settings):array
	{
		...
		$data = [
			...
			'art_kategorie' 	=> $this->elasticExportHelper->getCategory((int)$item->variationStandardCategory->categoryId, $settings->get('lang'), $settings->get('plentyId')),
			...
			'art_marke'			=> substr(trim($this->elasticExportHelper->getExternalManufacturerName((int)$item->itemBase->producerId)), 0, 20),
			'art_farbe' 		=> [],
			'art_groesse'		=> [],
