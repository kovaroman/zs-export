	...
	private function getMain(Record $item, KeyValue $settings):array
	{
		...
		$data = [
			...
			'ProductCategory' 		=> str_replace(array('&', '/'), array('und', ' '),
											$this->elasticExportHelper->getCategory((int)$item->variationStandardCategory->categoryId, $settings->get('lang'), $settings->get('plentyId'))),
			...
			'BrandName'				=> $this->elasticExportHelper->getExternalManufacturerName((int)$item->itemBase->producerId),
			'Price'					=> number_format((float)$this->elasticExportHelper->getPrice($item), 2, ',', ''),
			'PreviousPrice'			=> number_format((float)$this->elasticExportHelper->getRecommendedRetailPrice($item, $settings) > $this->elasticExportHelper->getPrice($item) ? $this->elasticExportHelper->getRecommendedRetailPrice($item, $settings) : 0, 2, ',', ''),
