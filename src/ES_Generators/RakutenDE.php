...
class RakutenDE extends CSVGenerator
{
	...
	private function buildRows($settings, $variations)
	{
		if (is_array($variations) && count($variations) > 0)
		{
			...
			foreach($variations as $key => $variation)
			{
				...
				{
					$this->attributeName[$variation['data']['item']['id']] = $this->elasticExportHelper->getEsAttributeName($variation, $settings);
			...
			foreach($variations as $key => $variation)
			{
				 ...
				 */
				$attributeValue = $this->elasticExportHelper->getEsAttributeValueSetShortFrontendName($variation, $settings, '|', $this->attributeNameCombination[$variation['data']['item']['id']]);
	...
	private function buildParentWithoutChildrenRow($item, KeyValue $settings)
	{
		...
		$data = [
			'id'						=> '',
			'variante_zu_id'			=> '',
			'artikelnummer'				=> $this->elasticExportHelper->generateEsSku($item['id'], 106, 0, $item['data']['skus']['sku']),
			'produkt_bestellbar'		=> $stockList['variationAvailable'],
			'produktname'				=> $this->elasticExportHelper->getEsName($item, $settings, 150),
			'hersteller'				=> $this->elasticExportHelper->getExternalManufacturerName($item['data']['item']['manufacturer']['id']),
			'beschreibung'				=> $this->elasticExportHelper->getEsDescription($item, $settings, 5000),
			'variante'					=> $this->attributeName[$item['data']['item']['id']],
			'variantenwert'				=> '',
			'isbn_ean'					=> $this->elasticExportHelper->getEsBarcodeByType($item, $settings->get('barcode')),
			...
			'kategorien'				=> $this->elasticExportHelper->getCategory((int)$item['data']['defaultCategories']['id'], $settings->get('lang'), $settings->get('plentyId')),
			'lieferzeit'				=> $this->elasticExportHelper->getEsAvailability($item, $settings, false),
			'tradoria_kategorie'		=> $item['data']['item']['rakutenCategoryId'],
			'sichtbar'					=> 1,
			'free_var_1'				=> $this->idlVariations[$item['id']]['itemBase.free1'],
			'free_var_2'				=> $this->idlVariations[$item['id']]['itemBase.free2'],
			'free_var_3'				=> $this->idlVariations[$item['id']]['itemBase.free3'],
			'free_var_4'				=> $this->idlVariations[$item['id']]['itemBase.free4'],
			'free_var_5'				=> $this->idlVariations[$item['id']]['itemBase.free5'],
			'free_var_6'				=> $this->idlVariations[$item['id']]['itemBase.free6'],
			'free_var_7'				=> $this->idlVariations[$item['id']]['itemBase.free7'],
			'free_var_8'				=> $this->idlVariations[$item['id']]['itemBase.free8'],
			'free_var_9'				=> $this->idlVariations[$item['id']]['itemBase.free9'],
			'free_var_10'				=> $this->idlVariations[$item['id']]['itemBase.free10'],
			'free_var_11'				=> $this->idlVariations[$item['id']]['itemBase.free11'],
			'free_var_12'				=> $this->idlVariations[$item['id']]['itemBase.free12'],
			'free_var_13'				=> $this->idlVariations[$item['id']]['itemBase.free13'],
			'free_var_14'				=> $this->idlVariations[$item['id']]['itemBase.free14'],
			'free_var_15'				=> $this->idlVariations[$item['id']]['itemBase.free15'],
			'free_var_16'				=> $this->idlVariations[$item['id']]['itemBase.free16'],
			'free_var_17'				=> $this->idlVariations[$item['id']]['itemBase.free17'],
			'free_var_18'				=> $this->idlVariations[$item['id']]['itemBase.free18'],
			'free_var_19'				=> $this->idlVariations[$item['id']]['itemBase.free19'],
			'free_var_20'				=> $this->idlVariations[$item['id']]['itemBase.free20'],
			...
			'technical_data'			=> $this->elasticExportHelper->getEsTechnicalData($item, $settings),
	...
	{
		$imageList = $this->elasticExportHelper->getEsImageList($item, $settings);
    ...
    private function getPriceList($item, KeyValue $settings):array
    {
        ...
        $variationRrp = $this->elasticExportHelper
            ->getEsRecommendedRetailPrice($this->idlVariations[$item['id']]['variationRecommendedRetailPrice.price'], $settings);
        $variationSpecialPrice = $this->elasticExportHelper
            ->getEsSpecialPrice($this->idlVariations[$item['id']]['variationSpecialOfferRetailPrice.retailPrice'], $settings);
            ...
            foreach($idlResultList as $idlVariation)
            {
                if($idlVariation instanceof Record)
                {
                    $this->idlVariations[$idlVariation->variationBase->id] = [
                        ...
                        'itemBase.free1' => $idlVariation->itemBase->free1,
                        'itemBase.free2' => $idlVariation->itemBase->free2,
                        'itemBase.free3' => $idlVariation->itemBase->free3,
                        'itemBase.free4' => $idlVariation->itemBase->free4,
                        'itemBase.free5' => $idlVariation->itemBase->free5,
                        'itemBase.free6' => $idlVariation->itemBase->free6,
                        'itemBase.free7' => $idlVariation->itemBase->free7,
                        'itemBase.free8' => $idlVariation->itemBase->free8,
                        'itemBase.free9' => $idlVariation->itemBase->free9,
                        'itemBase.free10' => $idlVariation->itemBase->free10,
                        'itemBase.free11' => $idlVariation->itemBase->free11,
                        'itemBase.free12' => $idlVariation->itemBase->free12,
                        'itemBase.free13' => $idlVariation->itemBase->free13,
                        'itemBase.free14' => $idlVariation->itemBase->free14,
                        'itemBase.free15' => $idlVariation->itemBase->free15,
                        'itemBase.free16' => $idlVariation->itemBase->free16,
                        'itemBase.free17' => $idlVariation->itemBase->free17,
                        'itemBase.free18' => $idlVariation->itemBase->free18,
                        'itemBase.free19' => $idlVariation->itemBase->free19,
                        'itemBase.free20' => $idlVariation->itemBase->free20,
                        'variationBase.id' => $idlVariation->variationBase->id,
                        'itemPropertyList' => $idlVariation->itemPropertyList, //todo further adjustments maybe needed because of a list
