...
class GoogleShopping extends CSVGenerator
{
	...
	protected function generateContent($resultList, array $formatSettings = [])
	{
		...
		foreach($resultList['documents'] as $variation)
		{
			...
			$salePrice = number_format((float)$this->elasticExportHelper->getEsSpecialPrice($this->idlVariations[$variation['id']]['variationSpecialOfferRetailPrice.retailPrice'], $settings), 2, '.', '');
			...
			$shippingCost = $this->elasticExportHelper->getEsShippingCost($variation['data']['item']['id'], $settings);
			...
			$imageList = $this->elasticExportHelper->getEsImageListInOrder($variation, $settings, 1, 'variationImages');

			$data = [
				'id' 						=> $variation['data']['item']['id'],
				'title' 					=> $this->elasticExportHelper->getEsName($variation, $settings, 256),
				...
				'availability'				=> $this->elasticExportHelper->getEsAvailability($variation['data']['defaultCategories'][0]['id'], $settings, false),
				'price'						=> $variationPrice,
				'sale_price'				=> $salePrice,
				'brand'						=> $this->elasticExportHelper->getExternalManufacturerName((int)$variation['data']['item']['manufacturer']['id']),
				'gtin'						=> $this->elasticExportHelper->getEsBarcodeByType($variation, $settings->get('barcode')),
				'isbn'						=> $this->elasticExportHelper->getEsBarcodeByType($variation, ElasticExportHelper::BARCODE_ISBN),
	...
	private function getItemPropertyList($variation):array
	{
		...
		{
			$characterMarketComponentList = $this->elasticExportHelper->getEsItemCharactersByComponent($this->idlVariations[$variation['data']['item']['id']], 143.00);
	...
	private function getIdentifierExists($variation, KeyValue $settings):string
	{
		...
		if (	strlen($this->elasticExportHelper->getEsBarcodeByType($variation, $settings->get('barcode'))) > 0 ||
				strlen($this->elasticExportHelper->getEsBarcodeByType($variation, ElasticExportHelper::BARCODE_ISBN)) > 0 )
		{
			$count++;
		}
	...
	{
		$description = $this->elasticExportHelper->getEsItemCharacterByBackendName($variation, $settings, self::CHARACTER_TYPE_DESCRIPTION);

		if (strlen($description) <= 0)
		{
			$description = $this->elasticExportHelper->getEsDescription($variation, $settings, 5000);
		}
